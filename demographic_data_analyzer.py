...
def calculate_demographic_data(print_data=True):
    ...
    adv_ed_rich = higher_education[higher_education['salary'] == '>50K']

    lower_education = (df[~df['education'].isin(['Bachelors', 'Masters', 'Doctorate'])])
    adv_ed_not = lower_education[lower_education['salary'] == '>50K']
    # percentage with salary >50K
    higher_education_rich = round(((len(adv_ed_rich)) / len(higher_education)) * 100, 1)
    lower_education_rich = round(((len(adv_ed_not)) / len(lower_education)) * 100, 1)
